...
- (void)configureCellWithObject:(STDemoDoctor *)anObj
{
    NSAssert([anObj isKindOfClass:[STDemoDoctor class]], @"所传递的实体类不正确");
    
    self.gravatarImageView.image = [UIImage imageNamed:anObj.gravatar];
    self.nameLabel.text          = anObj.name;
    self.typeLabel.text          = anObj.type;
    self.hospitalLabel.text      = anObj.hospital;
    self.goodatLabel.text        = anObj.goodat;
}
